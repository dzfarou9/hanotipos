...
التطبيق يعمل على ويندوز بأسلوب **سحابي أولاً مع بديل محلي (Cloud-First, Local Fallback)**:

- **المصادقة:** دخول سحابي بنفس حساب الهاتف (`phone@hanoti.pos` في Firebase Auth) — تعمل المزامنة والاشتراك السحابي. عند تعذر Firebase (غير متصل/غير مهيأ) يُقبل حساب محلي مشفّر (sha256 + ملح) عبر `flutter_secure_storage`.
- **الاشتراك:** حكم السيرفر أولاً عند وجود جلسة؛ وبديل محلي (30 يوماً) عند العمل دون اتصال.
- **الماسح:** قارئ USB (keyboard wedge) عبر `BarcodeInputService` بدل كاميرا/ML Kit؛ تسجيل الدخول بـ QR معطّل.
- **ملاحظة App Check:** إن كان App Check مفعّلاً بوضع "Enforced" على Firestore في Firebase Console، فطلبات ويندوز سترفض — اتركه "Unenforced" أو سجّل رمز Debug.
